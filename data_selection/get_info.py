...
from data_selection.get_validation_dataset import (get_dataloader,
                                                        get_dataset)
from data_selection.get_test_dataset import get_test_dataset
...
parser.add_argument('--task_split', type=str, choices=['test', 'val'], default='val',
                    help='Specify the task split to be test or validation')
...
parser.add_argument("--base_model_path", type=str,
                    default=None, help="The path to the base model")
parser.add_argument("--max_samples", type=int,
                    default=None, help="The maximum number of samples")
parser.add_argument("--torch_dtype", type=str, default="bfloat16",
                    choices=["float16", "bfloat16"], help="The torch data type")
...
if args.task is not None:
    if args.task_split == 'val':
        dataset = get_dataset(args.task,
                              data_dir=args.data_dir,
                              tokenizer=tokenizer,
                              chat_format=args.chat_format,
                              use_chat_format=not args.no_use_chat_format,
                              max_length=args.max_length,
                              zh=args.zh)
    elif args.task_split == 'test':
        dataset = get_test_dataset(args.task,
                                   data_dir=args.data_dir,
                                   tokenizer=tokenizer,
                                   chat_format=args.chat_format,
                                   use_chat_format=not args.no_use_chat_format,
    ...
    dataset = get_training_dataset(
        args.train_file, tokenizer, args.max_length, sample_percentage=1.0, qwen=qwen)
