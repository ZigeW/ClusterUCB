...
def parse_args():
    ...
    argparser.add_argument('--score_path', type=str, default=None)
    ...
    # argparser.add_argument("--ckpt", type=int)
...
if __name__ == "__main__":
    ...
    if args.score_path is None:
        args.score_path = args.output_path
    ...
    for target_task in args.target_task_names:
        ...
        score_paths = [os.path.join(
            args.score_path, task_name, target_task, f"influence_score.pt") for task_name in args.train_file_names]
        num_samples = []
        ...
        topk_scores, topk_indices = torch.topk(
            all_scores.float(), args.max_samples, dim=0, largest=not args.reverse)
        ...
        if not args.reverse:
            ...
            file_name = os.path.join(output_path, f"top_{data_amount_name}.jsonl")
        else:
            final_index_list = sorted_index[-args.max_samples:].tolist()
            final_data_from = data_from[-args.max_samples:].tolist()
            file_name = os.path.join(output_path, f"bottom_{data_amount_name}.jsonl")
