...
def collect_grads(dataloader,
    ...
    project_interval = 8  # project every 16 batches
    ...
    for batch in tqdm(dataloader, total=len(dataloader)):
        ...
        try:
            ...
            invalid_list.append(count-1)
            full_grads.append(torch.zeros_like(vectorized_grads))
            model.zero_grad()
            print(f"Invalid sample {count-1}: token length {batch['input_ids'].shape[-1]}.")

        if count % project_interval == 0 and len(full_grads) != 0:
            _project(full_grads, projected_grads)
            full_grads = []

        if count % save_interval == 0 and len(projected_grads) != 0:
    ...
    outfile = os.path.join(output_dir, f"invalid_points.pt")
    torch.save(invalid_list, outfile)
    print(f"Finished. Invalid sample: {len(invalid_list)}")
...
def get_loss(dataloader: torch.utils.data.DataLoader,
    ...
    torch.save(total_loss, os.path.join(output_dir, "loss.pt"))
