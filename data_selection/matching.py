...
argparser.add_argument('--method', type=str, default='similarity', choices=['similarity', 'L2'])
...
def calculate_influence_score(training_info: torch.Tensor, validation_info: torch.Tensor, method):
    ...
    if method == 'similarity':
        ...
        influence_scores = torch.cdist(training_info, validation_info, p=2)
...
for target_task_name in args.target_task_names:
    for train_file_name in args.train_file_names:
        ...
        for i, ckpt in enumerate(args.ckpts):
            # validation_path = args.validation_gradient_path.format(ckpt)
            ...
            influence_score += args.checkpoint_weights[i] * \
                calculate_influence_score(
                    training_info=training_info, validation_info=validation_info, method=args.method)
        ...
        output_file = os.path.join(
            args.output_path, f"{train_file_name}", target_task_name, f"influence_score.pt")
