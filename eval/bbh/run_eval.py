...
exact_match = evaluate.load("./eval/evaluate-hf/metrics/exact_match/exact_match.py", module_type="metric")
...
def main(args):
    ...
    for task_name in tqdm.tqdm(all_tasks.keys(), desc="Evaluating"):
        ...
        if args.model_name_or_path:
            ...
            # generate with hf model
                ...
                stop_sequence = tokenizer.encode("\n\n", add_special_tokens=False)[
                                -2:]  # get the last token because the tokenizer may add space tokens at the start.
                outputs = generate_completions(
                    ...
                    stop_id_sequences=[[stop_sequence] + [tokenizer.encode(stop, add_special_tokens=False) for stop in
        ...
        exact_match.compute(predictions=predictions, references=targets, ignore_case=True, ignore_punctuation=True)[
            "exact_match"]
