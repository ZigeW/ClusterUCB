...
def create_prompt_with_llama3_chat_format(messages, tokenizer, bos="<s>", eos="</s>", add_bos=True):
    '''
    This function is adapted from the official llama3 tokenizer script:
    ...
    def encode_message(message):
        return encode_header(message) + message["content"].strip() + "<|eot_id|>"

    formatted_text = "<|begin_of_text|>"
    for message in messages:
        formatted_text += encode_message(message)
    # Add the start of an assistant message for the model to complete.
    formatted_text += encode_header({"role": "assistant", "content": ""})
    return formatted_text
