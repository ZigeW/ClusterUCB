...
encoding_templates_with_context = {
    "english": (
    "Answer the following question based on the information in the given passage.", "Passage:", "Question:", "Answer:"),
    ...
    "indonesian": (
    "Jawab pertanyaan berikut berdasarkan informasi di bagian yang diberikan.", "Bagian:", "Pertanyaan:", "Jawaban:"),
...
def main(args):
    ...
    if args.model_name_or_path:
        if args.use_vllm:
            ...
            stop_sequences = [[tokenizer.encode(stop_sequence, add_special_tokens=False)[-1]] for stop_sequence in
                              args.additional_stop_sequence]
            # we only use stop token for non-chat format (usually applied to vanilla pretrained language models).
            # For chat format, we will rely on the model knows when to stop.
            if not args.use_chat_format:
    ...
    metric = evaluate.load("./eval/evaluate-hf/metrics/squad/squad.py", module_type="metric")

    eval_scores = {}
    for lang in data_languages:
        lang_predictions = [{"id": example["id"], "prediction_text": output} for example, output in
                            zip(test_data, outputs) if example["lang"] == lang]
        lang_references = [{"id": example["id"], "answers": example["answers"]} for example in test_data if
                           example["lang"] == lang]
    ...
    eval_scores["average"] = {metric: np.mean([scores[metric] for scores in eval_scores.values()]) for metric in
                              ["f1", "exact_match", "recall"]}
