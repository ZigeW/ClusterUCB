...
def get_training_dataset(train_files: List[str], tokenizer, max_seq_length, qwen=False, sample_size=None, sample_percentage=1.0, seed=0):
...
def load_raw_dataset(train_files: Union[List[str], str], sample_size=None, sample_percentage=1.0, seed=0):
    ...
    for train_file in train_files:
        processed_dataset = load_dataset(
            ...
            cache_dir="/data/wangzige/wangzige/cache/huggingface/datasets"
        )["train"]
        if sample_size is None:
            dataset_size = int(len(processed_dataset) * sample_percentage)
        else:
        ...
        with temp_seed(seed):
            ...
            processed_datasets.append(sampled_dataset)

    sampled_datasets = concatenate_datasets(processed_datasets)
    if sample_percentage < 1.0 or sample_size is not None:
        with temp_seed(seed):
            index = np.random.permutation(len(sampled_datasets))
        sampled_datasets = sampled_datasets.select(index)

    return sampled_datasets


def encode_data(raw_datasets, tokenizer, max_seq_length, qwen=False, processing_num_workers=10, overwrite_cache=False, func_name="encode_with_messages_format"):
    ...
    if "input_ids" in raw_datasets.features:
        return raw_datasets
    print(f"If Qwen = {qwen}")
    encode_function = get_encode_function(
        raw_datasets, tokenizer, max_seq_length, qwen=qwen, func=func_name)
...
def get_encode_function(raw_datasets, tokenizer, max_seq_length, qwen=False, func="encode_with_messages_format"):
...
def encode_with_messages_format(example, tokenizer, max_seq_length, qwen=False):
    '''
    ...
    example_text = concat_messages(messages, tokenizer, qwen)
    tokenized_example = tokenizer(
        example_text, return_tensors='pt', max_length=max_seq_length, truncation=True)
    input_ids = tokenized_example.input_ids
    labels = input_ids.clone()
        ...
        if message["role"] != "assistant":
            if message_idx == 0:
                ...
                message_start_idx = tokenizer(
                    concat_messages(messages[:message_idx], tokenizer, qwen), return_tensors='pt', max_length=max_seq_length, truncation=True
                ).input_ids.shape[1]
            if message_idx < len(messages) - 1 and messages[message_idx+1]["role"] == "assistant":
                # here we also ignore the role of the assistant
                messages_so_far = concat_messages(
                    messages[:message_idx+1], tokenizer, qwen) + "<|assistant|>\n"
            else:
                messages_so_far = concat_messages(
                    messages[:message_idx+1], tokenizer, qwen)
...
def concat_messages(messages, tokenizer, qwen=False):
    message_text = ""
    for message in messages:
        ...
        if qwen:  ## Qwen fine-tuning needs to remove the \n at the end!!!
