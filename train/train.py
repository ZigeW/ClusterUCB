...
class DynamicCallback(TrainerCallback):
...
def main():
    ...
    # If is LlamaTokenizer, add eos token explicityly
    with open(os.path.join(model_args.model_name_or_path, 'tokenizer_config.json'), 'r') as f:
    ...
    train_dataset = get_training_dataset(data_args.train_files,
                                         ...
                                         # sample_size=data_args.examples,
    ...
    if training_args.dynamic_train:
        trainer.add_callback(DynamicCallback(trainer))
    if training_args.warmup_train:
        trainer.add_callback(StopAfterSaveCallback(stop_step=training_args.save_steps))
